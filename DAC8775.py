...
from db import db_table_data_to_dictionary
...
class DAC(object):
    ...
    def dac_initialize(self):
        ...
        chipid = self.dac_read_register('device_ID', 'tblAdcIdReg')
        print(chipid)
        ...
        """ Read reset config  """
        resetconfig = self.dac_read_register('dac_read_reset_config_reg', 'tblDacResetConfigReg')
        print(resetconfig)

        """ Write Select Buck Boost Register (Select A,B,C & D) """
        self.dac_write_register('Select_Buck_Boost_converter', 'tblDacBuckBoostReg',
                                dca=True, dcb=True, dcc=True, dcd=True)
        """ Read Select Buck Boost Register """
        buckboostsel = self.dac_read_register('Select_Buck_Boost_converter', 'tblDacBuckBoostReg')
        print(buckboostsel)

        """ Write Config Buck-Boost reg """
        self.dac_write_register('configuration_Buck_Boost_converter', 'tblDacConfigBuckBoostReg',
                                pnsel=3, nclmp=7, pclmp=8, cclp=0)
        """ Read Config Buck Boost Register """
        buckboostconfig = self.dac_read_register('dac_read_config_buck_boost_reg', 'tblDacConfigBuckBoostReg')
        print(buckboostconfig)

        """ Write Select DAC Register """
        self.dac_write_register('select_dac', 'tblDacSelectDacReg',
        ...
        dacsel = self.dac_read_register('dac_read_select_dac_reg', 'tblDacSelectDacReg')
        print(dacsel)

        """ Write Config Dac Register"""
        self.dac_write_register('configuration_dac', 'tblDacConfigDacReg',
                                range=6, sren=False, sr_step=0, srclk_rate=0xf,
                                oten=False, hten=False, sclim=0)
        """ Read Config Dac Register """
        daccnfg = self.dac_read_register('configuration_dac', 'tblDacConfigDacReg')
        print(daccnfg)
    ...
    def dac_write_register(self, regname, tblname, **kwargs):
        reg_dict = db_table_data_to_dictionary(tblname)
    ...
    def dac_read_register(self, reg_name, tblname):
    ...
    def dac_aliveness_check(self):
        self.dac_write_register('reset_config', 'tblDacResetConfigReg',
                                ...
                                clrena=False, clrenb=False, clrenc=False, clrend=False)
        reg_dict = self.dac_read_register('status', 'tblAdcStatusReg')
